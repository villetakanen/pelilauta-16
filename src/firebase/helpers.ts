...
export function extractFlowTime(data: any): number {
  if (!data) return -1
  if (data.flowTime && typeof data.flowTime === 'number') return data.flowTime as number
  if (data.updatedAt && data.updatedAt.toMillis()) return Math.floor(data.updatedAt.toMillis()) as number
  if (data.createdAt && data.createdAt.toMillis()) return Math.floor(data.createdAt.toMillis()) as number
  return -1
}