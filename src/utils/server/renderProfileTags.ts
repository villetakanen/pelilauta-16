...
export const renderProfileTags = (markdown: string, origin: string) => {
  if (!origin) {
  ...
  for (const match of matches) {
    ...
    const newLink = `${origin}/profiles/${encodeURIComponent(tagName.toLowerCase())}`;
