...
test('getMarked with site returns a function that can render in-site links', async () => {
  const marked = await getMarkedInstance('https://example.com', {
    site: mockSite 
  });
  
  const result = marked.parse('[Test Link](test-link)');
  expect(result).toContain('<a href="https://example.com/sites/test-site/test-link">Test Link</a>');
});
...
test('getMarked with site returns a function that can render wikilink shortcuts', async () => {
  ...
  expect(result).toContain('<a href="https://example.com/sites/test-site/test-link">obsidian style link</a>');
});
